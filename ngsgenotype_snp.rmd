...
loadedData75 <- LoadAMData2(QCGenotypes, targetStatus = c("High Quality")) # Load all samples with 75 or more working loci

# if an allele is fixed you can get an error here: 'x' must be atomic
...
processedGenotypes$Status[processedGenotypes$Sample %in% uniqueBears75$unclassified$index] <- "Unclassified"
...
processedGenotypes$Status[processedGenotypes$Sample %in% uniqueBears75$multipleMatches$index] <- "MultipleMatched"

# Cluster all except the removed samples:
loadedDataAll <- LoadAMData2(processedGenotypes, targetStatus = c("Medium Quality", "High Quality")) # Load all samples with 67 or more working loci
...
loadedDataFilt <- LoadAMData2(finalGenotypes, targetStatus = c("Medium Quality", "High Quality")) # Load all samples with 67 or more working loci
...
allelematch::summary.amUnique(object = uniqueBearsFilt, csv = "unique_bears_filt_10_v1.csv")

table(finalGenotypes$Status) # summarise statuses of samples
View(read.table("unique_bears_filt_10_v1.csv", sep = ",", header = TRUE))
```
