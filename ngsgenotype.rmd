...
RunLocus <- function(locusName, allFiles, filteredFolder) {
  ...
  gentypRes <- PlotAllSamples(loadedFiles = loadedFastqFiles, writeTo = paste0("Plots/", locusName, ".pdf"), primerInfo = primerDF, fragRange = 10:700)
...
RunAllLoci <- function(inputFiles, filteredFolder) {
  lociNames <- unique(gsub("_.+", "", inputFiles))
  genotypesList <- lapply(lociNames, RunLocus, inputFiles, filteredFolder)
  allGenotypes <- Reduce(function(x, y) merge(x, y, by = "Sample"), genotypesList)
  return(allGenotypes)
}

# Load all samples and calculate all loci:
allGenotypes <- RunAllLoci(inputFiles, filteredFolder)

# Write all genotypes to an exce-sheet:
library(writexl)
